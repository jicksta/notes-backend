...
EvernoteAPI.prototype = {
  ...
  notebook: function(notebookGUID) {
    return promiseTranslator(function(callback) {
      this.notestore().getNotebook(notebookGUID, callback);
    }, this);
  },
  ...
  tag: function(tagGUID) {
    return promiseTranslator(function(callback) {
      this.notestore().getTag(tagGUID, callback);
    }, this);
  },
  ...
  updateNote: function(note) {
    if(!(note instanceof Evernote.Note)) note = NoteTransformer.toThrift(note);
    return promiseTranslator(function(callback) {
      this.notestore().updateNote(note, callback);
    }, this);
  },

  updateNotebook: function(notebook) {
    if(!(notebook instanceof Evernote.Notebook)) notebook = NotebookTransformer.toThrift(notebook);
    return promiseTranslator(function(callback) {
      this.notestore().updateNotebook(notebook, callback);
    }, this);
  },

  updateTag: function(tag) {
    if(!(tag instanceof Evernote.Tag)) tag = TagTransformer.toThrift(tag);
    return promiseTranslator(function(callback) {
      this.notestore().updateTag(tag, callback);
    }, this);
  },
