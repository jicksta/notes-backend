...
EvernoteAPI.prototype = {
  ...
  createNotebook: function(name, options) {
    var blueprint = new Evernote.Notebook(optionsFromDefaults(options, {name: name, stack: null}));
    return promiseTranslator(function(callback) {
      this.notestore().createNotebook(blueprint, callback);
    }, this);
  },

  createTag: function(name) {
    var blueprint = new Evernote.Tag({name: name});
    return promiseTranslator(function(callback) {
      this.notestore().createTag(blueprint, callback);
    }, this);
  },

  createNote: function(options) {
    var attrs = _.pick(options, 'title', 'content');
    attrs.tagNames = options.tags;
    attrs.tagGuids = options.tag_ids;
    attrs.notebookGuid = options.notebook_id;

    var blueprint = new Evernote.Note(attrs);
    return promiseTranslator(function(callback) {
      this.notestore().createNote(blueprint, callback);
    }, this);
  },
  ...
  note: function(noteGUID) {
    return promiseTranslator(function(callback) {
      this.notestore().getNote(noteGUID, true, false, true, true, callback);
    }, this);
  },
  ...
  tags: function() {
    return promiseTranslator(function(callback) {
      this.client().getNoteStore().listTags(callback);
    }, this);
  },
