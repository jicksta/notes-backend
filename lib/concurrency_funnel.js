...
ConcurrencyFunnel.prototype = {
  ...
  _promiseStarted: function(promise) {
    ...
    this.numRunning++;
    //console.log("Promise started  | Running: %d | Remaining: %d", this.numRunning, this.remaining.length);
  },

  _promiseFinished: function() {
    this.numRunning--;
    //console.log("Promise finished | Running: %d | Remaining: %d", this.numRunning, this.remaining.length);
    if(!this.atCapacity()) {
      this._startNext();
    }
    if(this.numRunning === 0) {
      //console.log("ALL FINISHED     | Running: %d | Remaining: %d", this.numRunning, this.remaining.length);
